...
def byte_level_tokenizer():
    ...
    def decode(byte_values):
        return bytes(byte_values).decode("utf-8")

    def adjacent_token_counter(token_list: list[int]):
    ...
    def merge(token_list : list[int], pair: tuple[int, int], new_id : int):
        idx = 0
        while idx < len(token_list) - 1:
    ...
    def most_freq_finder(adjacent_token_count : dict[tuple[int, int], int]):
    ...
    def decode_most_freq(token_list : list[int], merged_pair : dict[tuple[int, int], int]):
